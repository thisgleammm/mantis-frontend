  ...
  return (
    <div className="min-h-screen px-6 py-10 font-sans bg-gray-50 text-black dark:bg-black dark:text-white">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10 mt-4">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm mb-6 text-gray-500">{product.description}</p>
              ...
              {product.variants && product.variants.length > 0 && (
                <div className="mb-6">
                  <p className="text-sm mb-2 text-gray-500 dark:text-gray-400">Pilih Varian:</p>
            ...
            <div className="flex gap-3 mt-6">
              <button className="flex-1 py-3 rounded-xl border text-sm transition border-black/15 text-black hover:bg-black/5 dark:border-white/15 dark:text-white dark:hover:bg-white/10">
                + Tambah ke Cart
              </button>
              <button className="flex-1 py-3 rounded-xl text-sm font-semibold transition bg-black text-white hover:bg-zinc-800 dark:bg-white dark:text-black dark:hover:bg-gray-200">
                Beli Sekarang
              </button>
            </div>
          </div>
        ...
        <div className="mt-16">
          <h2 className="text-xl font-semibold mb-6 text-black dark:text-white">Reviews</h2>
          {product.reviews && product.reviews.length > 0 ? (
            <div className="flex flex-col gap-4">
              {product.reviews.map(review => (
                <div key={review.id} className="border rounded-2xl p-4 bg-white border-black/8 dark:bg-white/4 dark:border-white/8">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-yellow-400 text-sm">{"⭐".repeat(review.rating)}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500">{review.created_at?.slice(0, 10)}</span>
                  </div>
            ...
            <p className="text-sm text-gray-400 dark:text-gray-600">Belum ada review untuk produk ini.</p>
          )}
        </div>
