        ...
        <div className="min-h-screen bg-black text-white px-6 py-10 font-sans">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-500 text-sm mt-1">{dummyOrders.length} orders ditemukan</p>
                </div>
                    ...
                    <div className="flex flex-col gap-5">
                        {dummyOrders.map(order => {
                            ...
                            return (
                                <div
                                    key={order.id}
                                    className="bg-white/4 border border-white/8 rounded-2xl p-6 hover:border-white/15 transition"
                                >
                                    {/* Top Row */}
                                    ...
                                    <div className="border-t border-white/8 pt-4 flex items-center justify-between">
                                        <div className="text-sm text-gray-500">
                                            {order.courier_name && (
                                                <span>{order.courier_name} {order.tracking_number ? `• ${order.tracking_number}` : ""}</span>
                                            )}
                                        </div>
                                        <div className="text-right">
                                            <p className="text-xs text-gray-500 mb-1">Total</p>
                                            <p className="text-base font-bold">Rp {order.grand_total.toLocaleString("id-ID")}</p>
                                        </div>
                                    </div>
                                </div>
                            )
